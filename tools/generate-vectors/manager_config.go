...
func managerConfigV2EveryKnob() map[string]any {
	return map[string]any{
		...
		"permissions":           map[string]any{"companyA": "yolo", "personal": "native"},
		...
		"system_module_waivers": []any{
			map[string]any{
				"package": "sysleaf", "reason": "reviewed leaf system prompt; admitted for this profile",
			},
		},
		"backup_retention":        0,
...
func managerConfigV2SchemaExamples(valid map[string]any) []schemaExample {
	...
	return []schemaExample{
		...
		{name: "invalid-permissions-profile-grammar", instance: withKnob("permissions", map[string]any{"Company A": "yolo"})},
		...
		{name: "valid-system-module-waiver", valid: true, instance: withKnob("system_module_waivers", []any{map[string]any{"package": "sysleaf", "reason": "reviewed leaf system prompt"}})},
		{name: "invalid-transitive-system-modules-value", instance: withKnob("transitive_system_modules", "quarantine")},
		{name: "invalid-system-module-waiver-package-grammar", instance: withKnob("system_module_waivers", []any{map[string]any{"package": "Bad Name", "reason": "x"}})},
		{name: "invalid-system-module-waiver-missing-reason", instance: withKnob("system_module_waivers", []any{map[string]any{"package": "sysleaf"}})},
		{name: "invalid-system-module-waiver-unknown-field", instance: withKnob("system_module_waivers", []any{map[string]any{"package": "sysleaf", "reason": "x", "expires": "never"}})},
...
func managerConfigV2Vectors() []any {
	...
	return []any{
		...
		map[string]any{
			"name": "schema2-permissions", "input": withEnvironments(map[string]any{"permissions": map[string]any{"companyA": "yolo"}}), "valid": true,
			"expected": map[string]any{"environments": expectedWith(map[string]any{"permissions": map[string]any{"companyA": "yolo"}})},
		},
		map[string]any{"name": "schema2-permissions-value-grammar", "input": withEnvironments(map[string]any{"permissions": map[string]any{"companyA": "standard"}}), "valid": false},
		map[string]any{

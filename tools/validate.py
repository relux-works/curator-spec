...
# The schema-8 and descriptor schema-2 shapes the boundary reserves. They carry
# the REQUIRED and OPTIONAL toolchain requirement placement of decision 0007 and
# nothing else. They do not exist yet; the moment one is minted it must match.
RESERVED_WIRE_SHAPES: dict[str, tuple[tuple[str, ...], tuple[str, ...]]] = {
...
EXACT_PROPERTY_SCHEMAS: dict[str, dict[str, dict[str, str]]] = {
    **{name: {"toolchain": TOOLCHAIN_REQUIREMENT_REF} for name in RESERVED_WIRE_SHAPES},
...
# Schema files reserved by the boundary and deliberately not created here.
...
def validate_additional_driver_boundary() -> None:
    ...
    closed_shapes = {**CLOSED_DRIVER_SHAPES, **RESERVED_WIRE_SHAPES}
    for name, definition in sorted(definitions.items()):
    ...
    for name in sorted(RESERVED_WIRE_SHAPES):

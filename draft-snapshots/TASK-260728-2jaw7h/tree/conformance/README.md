...
- the compiled-build toolchain contract, which lives in `conformance/next`
  rather than here because its surface is not yet released (section 3.1): the
  three requirement kinds and their interval intersection, the manager-tested
  family gate evaluated after and independently of the requirement, Stage A's
  eight ordered steps with host-pair applicability ahead of declaration
  presence, origin admissibility, and filesystem shape, Stage B's five ordered
  steps with the descriptor-narrowed requirement re-evaluated before the
  file-shape gate and every disposition, the `go.mod` `go` and `toolchain` value
  classifiers against upstream's two-layer acceptance in both directions, the
  twelve `build_toolchain_*` diagnostics with one payload instance per firing
  site, the manager-owned registry and guidance catalog with their resolution,
  reachability and version-transition gates, and the identity guards proving a
  requirement, a tested-family set, or a guidance revision changes no cache key;
...
### 3.1 The candidate suite root

`conformance/next` is the second suite root. It carries the generated cases for
a surface that has been minted in `schemas/v1` but not yet released: currently
manifest schema 8, repository descriptor schema 2, and the three manager-owned
`toolchain-*` documents.

The split exists because `conformance/v1` is pinned byte-for-byte. A release
document names its suite root and the SHA-256 of that root's `manifest.json`, so
a new case written under `conformance/v1` changes the digest of an already
released candidate. Regenerating both the suite and the document that pins it
produces an internally consistent result and therefore hides the rewrite, which
is why `release/frozen.json` records each released artifact's accepted digest,
is authored rather than generated, and is checked by `tools/generate-vectors`,
`tools/validate.py`, and `tools/release_gate.py` alike.

`conformance/next/manifest.json` is held to the same inventory and digest
discipline as the released manifest, and deliberately carries no
`protocol_version`: naming one would mint one. It records the released version
it is a candidate against and the task that owns the mint instead. Promoting
these cases into a released suite root, and pinning them, belongs to that task.
`CURATOR_CONFORMANCE_ROOT` continues to name a released suite root only.

...
4a. run Stage A of the toolchain preflight of Protocol Core section 4.2.3.5 for
   every distinct toolchain in the plan, in Unicode-scalar lexical order of
   toolchain identifier and once per operation, before external repository
   acquisition, before build-cache lookup, and before any persistent mutation;
...
That rule is the `go` registry entry's `compatibility` set of Protocol Core
section 4.2.3.1 — families `{(1, 23)}` at `(major, minor)` granularity, extended
only by testing — and it is preserved rather than replaced. The registry
baseline `at_least 1.23.0` is the requirement a schema-6 or schema-7 command
inherits when it declares none, so those schemas gain the two-stage preflight
without gaining a field and without any currently admitted release becoming
inadmissible. The requirement and the `compatibility` set are independent gates:
passing either does not imply the other, and no package byte can widen or narrow
the set.

The manager resolves the toolchain root only through the two declaration
channels of Protocol Core section 4.2.3.3 — `operator_config` first, then
`bundled` — and never through the ambient `PATH`, an inherited environment
variable, a version-manager shim, a runtime root, a project directory, or any
package or repository byte. A missing declaration is
`build_toolchain_unavailable`; a declaration that exists and is not usable, for
any reason including a root that is absent on disk, is
`build_toolchain_untrusted`. The launcher is selected independently of the package and MUST be
...
`would-rebuild-untrusted-cache`, `corrupt`, `blocked`, or `unsupported`. A cache
miss is `would-preflight-and-build` because dry-run MUST NOT run `go list`,
`go build`, a compiler, or a linker.

A dry run runs both toolchain preflight stages, because neither needs a compiler
or a mutation, and reports a command whose toolchain fails either stage as
`blocked` with the typed `build_toolchain_*` diagnostic and its `guidance_id`.
`blocked` here has the same meaning as in section 11.7. `unsupported` continues
to mean an unknown driver and MUST NOT be reused for a toolchain failure.

Neither stage may be skipped by a cache hit, a dry run, or an offline mode. A
toolchain that fails Stage A fails the operation there, so cache lookup is never
reached and no candidate is consulted. A toolchain that passes Stage A and fails
Stage B fails there, still before the first candidate is read and before any
compiler child starts. A toolchain that passes both stages while a candidate was
built under a different toolchain identity is an ordinary miss, because the
resolved identity is already part of the cache key. The effective requirement,
the `compatibility` set, and the guidance catalog are gates rather than build
inputs: none of them enters a cache key, receipt, marker, or claim, so changing
one never invalidates an artifact.
...
5. parses the descriptor and validates the selected target/module containment,
   including a schema-2 target's OPTIONAL toolchain requirement;
6. applies allowlist, revocation, registry, tag-lock, and audit-policy gates
   independently to the external subject;
6a. runs Stage B of the toolchain preflight of Protocol Core section 4.2.3.6,
   whose first two steps re-compute the effective requirement with the
   descriptor requirement now joined and re-evaluate the resolved version
   against it; and
...
toolchain, or policy revision MUST NOT alias. The toolchain component of that
key is the resolved identity `{algorithm, version, primary_relpath,
content_sha256}` and never the declared requirement, so a requirement or
`compatibility` change alone produces the same key.
...
| `build_toolchain_requirement_invalid` | `schema` | `unsupported` | `error` | Wire toolchain requirement is malformed, names a non-primary identifier, or carries a value outside its closed grammar |
| `build_toolchain_requirement_unsatisfiable` | `schema` | `unsupported` | `error` | Baseline, manifest, and descriptor requirements intersect to the empty interval |
| `build_toolchain_unavailable` | `toolchain` | `blocked` | `error` | Neither declaration channel carries an entry for the identifier |
| `build_toolchain_untrusted` | `toolchain` | `blocked` | `error` | The channel entry, its holding state, or the filesystem object it names is inadmissible |
| `build_toolchain_version_undetermined` | `toolchain` | `blocked` | `error` | Probe output is unbounded, unmatched, or ambiguous |
| `build_toolchain_prerelease_unsupported` | `toolchain` | `blocked` | `error` | The resolved toolchain is a prerelease, nightly, beta, or development snapshot |
| `build_toolchain_platform_unsupported` | `toolchain` | `unsupported` | `error` | Host pair is outside the entry's platforms, or the reported target differs from the native target |
| `build_toolchain_incompatible` | `toolchain` | `blocked` | `error` | The resolved release is outside the effective requirement of its stage |
| `build_toolchain_untested_release` | `toolchain` | `blocked` | `error` | The resolved release family is outside the entry's manager-tested set |
| `build_toolchain_metadata_mismatch` | `descriptor` | `blocked` | `error` | Source metadata fails the ecosystem's grammar or asserts a version the resolved toolchain does not satisfy |
| `build_toolchain_package_influence_forbidden` | `descriptor` | `unsupported` | `error` | Source metadata carries a forbidden-disposition field or value class |
| `build_toolchain_changed` | `toolchain` | `corrupt` | `error` | Resolved toolchain identity or version changed during the operation |

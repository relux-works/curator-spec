...
- MUST NOT accept or derive package-controlled executable names, argument
  ...
  toolchain executables, roots, download URLs, mirrors, channels or tracks,
  version managers, install commands, credentials, keyrings, or trust roots,
  output names or paths, build scripts, pre/post-build actions, or
...
### Trusted toolchain selection boundary

The build command declares *which versions* of a toolchain it needs; the manager
alone decides *which toolchain* that is. `protocol/core.md` section 4.2.3 makes
that split checkable rather than aspirational.

A version constraint can only filter a manager-trusted set, so no manifest,
descriptor, or source file can add a candidate, a path, a URL, a channel, an
install command, or a trust root. The manager resolves a root through exactly
two declaration channels — trusted operator configuration held in owner-protected
manager state, and the toolchains shipped inside the manager distribution — and
through no other place. The ambient or user `PATH`, an inherited environment
variable such as `GOROOT`, `RUSTUP_HOME`, `CARGO_HOME`, `RUSTC`, `SWIFT_EXEC`,
`TOOLCHAINS`, `JAVA_HOME`, or `KOTLIN_HOME`, a runtime root, a project
directory, and any version-manager shim or wrapper including `rustup`, `asdf`,
`mise`, `sdkman`, `swiftly`, and `jenv` are all forbidden origins. An operator
MAY configure the concrete root a version manager produced; the manager resolves
that root directly and never through the shim.

The manager-tested `compatibility` set is what keeps "filter" from degenerating.
The trusted set is bounded by manager testing rather than by version ordering,
so no constraint spelling reaches a release the manager has never tested, and a
resolved release that satisfies every declared bound is still refused when its
family is outside that set.

Fingerprinting is honest about what it proves. It proves the toolchain tree is
stable across an operation and identical across operations; it does not prove
upstream authenticity. Verifying that the configured toolchain is genuinely the
vendor's remains the operator's responsibility at configuration time, and this
version performs no signature verification of a toolchain. A `content_sha256` in
a receipt MUST NOT be read as provenance.

Source-ecosystem metadata is read as an assertion and never as a selector. A
field naming *where* a toolchain comes from — an executable path, a toolchain
root, a URL, a mirror, a registry, a credential, an install command, an
environment override, or a trust root — is refused outright. A field asserting
*which version* one is claimed to be is compared against the already-resolved
toolchain and then discarded; reading a channel or track that way is exactly
what makes it harmless, because its native meaning as a selector is dropped.
...
`mise`, Homebrew, `winget`, the Gradle wrapper, or `GOTOOLCHAIN`. Refusing
auto-install keeps the largest untrusted action out of the operation, and
refusing a prerelease host keeps an unstable compiler identity out of a cache key
that promises reproducibility. Neither refusal claims that a trusted release
toolchain compiling adversarial source is safe: the compiler-input rejection
policy and the portable containment boundary below are unchanged and still apply.

Installation guidance is manager-owned. A diagnostic carries a `guidance_id`
into a versioned manager catalog whose `primary_source` is always a
manager-trusted origin — the language's own official origin, the manager's
operator documentation, or this specification — and never a package, a
repository, a mirror, a third-party installer script, or a command the manager
runs. Guidance is text plus a URL, the manager never executes it, and no skill
or repository can override catalog content.

The two preflight stages widen no boundary. Stage A runs earlier on inputs the
manager already trusted at that point — its own configuration and one validated
manifest field — and every gate that preceded compilation still precedes it.
Stage B runs only after local snapshot validation, or after exact external
acquisition and audit, and before the manager reads a cache candidate or starts
a compiler child.
...
review of complete compiler-visible input, a complete toolchain registry entry
with a closed source-metadata disposition table, trusted toolchain/sysroot
identity, fixed process graph/environment/arguments, offline dependency/link
policy,

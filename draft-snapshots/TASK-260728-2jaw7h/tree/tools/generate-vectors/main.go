...
const (
	...
	// candidateReleasePinOwner names the board task that mints the next release
	// and the pin that admits the candidate suite root. Nothing in this
	// generator advances a pin; the candidate manifest records who does.
	candidateReleasePinOwner = "TASK-260728-251p01"
)
...
func main() {
	...
	// The unreleased candidate suite root. conformance/v1 is pinned
	// byte-for-byte by release/1.0.0-rc.5.json, so material minted for the next
	// surface is generated here and stays out of the frozen manifest.
	candidate := filepath.Join(*root, "conformance", "next")
	fixture := filepath.Join(suite, "fixtures", "skill")
	expected := filepath.Join(suite, "expected")
	vectors := filepath.Join(suite, "vectors")
	candidateVectors := filepath.Join(candidate, "vectors")
	must(os.MkdirAll(filepath.Join(expected, "registry"), 0o755))
	must(os.MkdirAll(vectors, 0o755))
	must(os.MkdirAll(candidateVectors, 0o755))
	...
	writeToolchainVectors(candidateVectors)
	writeSchemaCases(suite, candidate, marker, ledger, audited, snapshot, entries[0], bundle, pinned)
	writeExternalRepositoryExpected(expected, marker)
	writeManifest(suite)
	writeCandidateManifest(candidate)
...
// candidateSchemaCases names every schema whose generated cases belong to the
// unreleased candidate suite root rather than to the frozen 1.0.0-rc.5 suite.
// conformance/v1 is pinned byte-for-byte by release/1.0.0-rc.5.json, so a case
// written under it moves a released identity; these six schemas are minted for
// the next surface and their cases are therefore written under conformance/next
// until TASK-260728-251p01 mints the release that admits them.
var candidateSchemaCases = map[string]bool{
	"agent-skill-v8.schema.json":                true,
	"csk-skill-v8.schema.json":                  true,
	"skill-build-v2.schema.json":                true,
	"toolchain-registry-v1.schema.json":         true,
	"toolchain-guidance-catalog-v1.schema.json": true,
	"toolchain-diagnostic-v1.schema.json":       true,
}

func writeSchemaCases(suite, candidate string, marker, ledger, audited, snapshot, logEntry, bundle map[string]any, pinned string) {
	...
	for _, prefix := range []string{"agent-skill", "csk-skill"} {
		name := prefix + "-v8.schema.json"
		manifest := validV8SkillManifest()
		cases[name] = schemaCase{manifest, without(manifest, "capabilities")}
		additionalCases[name] = v8SchemaExamples()
	}
	descriptorV2 := validSkillBuildV2()
	cases["skill-build-v2.schema.json"] = schemaCase{descriptorV2, without(descriptorV2, "targets")}
	additionalCases["skill-build-v2.schema.json"] = skillBuildV2SchemaExamples()
	registryDocument := toolchainRegistry()
	cases["toolchain-registry-v1.schema.json"] = schemaCase{registryDocument, without(registryDocument, "entries")}
	additionalCases["toolchain-registry-v1.schema.json"] = toolchainRegistrySchemaExamples()
	catalogDocument := guidanceCatalog()
	cases["toolchain-guidance-catalog-v1.schema.json"] = schemaCase{catalogDocument, without(catalogDocument, "entries")}
	additionalCases["toolchain-guidance-catalog-v1.schema.json"] = guidanceCatalogSchemaExamples()
	payloads := diagnosticPayloads()
	cases["toolchain-diagnostic-v1.schema.json"] = schemaCase{payloads[0], without(payloads[0], "violation")}
	additionalCases["toolchain-diagnostic-v1.schema.json"] = diagnosticSchemaExamples()

	frozenRoot := filepath.Join(suite, "schema-cases")
	candidateRoot := filepath.Join(candidate, "schema-cases")
	index := map[bool][]any{false: nil, true: nil}
	var names []string
	...
	for _, name := range names {
		isCandidate := candidateSchemaCases[name]
		root := frozenRoot
		if isCandidate {
			root = candidateRoot
		}
		...
		index[isCandidate] = append(index[isCandidate],
		...
		for _, example := range additionalCases[name] {
			...
			index[isCandidate] = append(index[isCandidate], map[string]any{
				"schema": name, "instance": filepath.ToSlash(filepath.Join(strings.TrimSuffix(name, ".schema.json"), filename)), "valid": example.valid,
			})
		}
	}
	writeJSON(filepath.Join(frozenRoot, "index.json"), index[false])
	writeJSON(filepath.Join(candidateRoot, "index.json"), index[true])
}
...
func manifestEntries(suite string) []any {
	var files []string
...
func writeManifest(suite string) {
	writeJSON(filepath.Join(suite, "manifest.json"), map[string]any{"protocol_version": protocolVersion, "generated_at": fixedTime, "generator": "tools/generate-vectors", "files": manifestEntries(suite)})
}

// writeCandidateManifest indexes the unreleased candidate suite root. It
// deliberately carries no protocol version: the surface it belongs to is
// reserved and unminted, and naming a version here would be minting one. It
// records the released version it is a candidate against and the task that owns
// the mint instead, so the document says what it is without claiming currency.
func writeCandidateManifest(candidate string) {
	writeJSON(filepath.Join(candidate, "manifest.json"), map[string]any{
		"released":          false,
		"candidate_against": protocolVersion,
		"release_pin_owner": candidateReleasePinOwner,
		"generated_at":      fixedTime,
		"generator":         "tools/generate-vectors",
		"files":             manifestEntries(candidate),
	})
}

// assertFrozenReleaseIdentity refuses to leave a rewritten frozen release
// behind. release/frozen.json is authored, never generated, so regenerating the
// corpus cannot move the expectation along with the artifact: a change that
// alters a released suite manifest, its schema-case index, or the release
// document that pins it fails here rather than producing a new, internally
// consistent, silently different release.
func assertFrozenReleaseIdentity(root string) {
	payload, err := os.ReadFile(filepath.Join(root, "release", "frozen.json"))
	must(err)
	var frozen struct {
		Releases []map[string]string `json:"releases"`
	}
	must(json.Unmarshal(payload, &frozen))
	if len(frozen.Releases) == 0 {
		must(fmt.Errorf("release/frozen.json records no frozen release"))
	}
	for _, record := range frozen.Releases {
		for _, subject := range [][2]string{
			{"release_document", "release_document_sha256"},
			{"suite_manifest", "suite_manifest_sha256"},
			{"schema_cases_index", "schema_cases_index_sha256"},
		} {
			relative, expected := record[subject[0]], record[subject[1]]
			...
			if actual != expected {
				must(fmt.Errorf(
					"generation rewrote frozen release %s: %s is %s, release/frozen.json requires %s",
					record["protocol_version"], relative, actual, expected,
				))
			}
		}
	}

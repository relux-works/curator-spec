...
4. computes `curator-build-source-v1` over every file, including
   `curator-build.json` and files outside the selected build root;
...
session, native target, fixed `go list`/`go build` vectors, compiler-visible
dependency containment, directive/native-input rejection, internal-link
policy, artifact validation, and no-execution rule. Only the selected external
build root is compiler-visible. No external source enters agent context,

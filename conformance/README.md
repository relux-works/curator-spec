...
- whole-snapshot audit-before-cache/compiler ordering for cache hit/miss,
  ...
  CCJ-1-derived receipt-v2/marker-v3 hashes, receipt-v1/v2 and marker-v2/v3
  mixed planning, rollback, status/repair/GC, shim/PATH, offline, and signing
  boundaries;
...
and 3. Claim v3 records driver/platform tuples in addition to the common
implementation, class, suite, time, and pass fields. Schema validity is not
qualification: every emitted tuple requires immutable native evidence against
the exact suite pin. The rc.5 specification candidate emits no manager claim;

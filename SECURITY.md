...
`skill-build.json`, external source/vendor/embed inputs, and diagnostics.
...
The six guarantees above are specified by the separately versioned hardened
execution profile, candidate `hardened-1.0.0-rc.1`, in
[`protocol/hardened-execution.md`](protocol/hardened-execution.md) and
[`profiles/manager-hardened.md`](profiles/manager-hardened.md). It is additive:
it changes no byte of `conformance/v1`, `schemas/v1`, or
[`release/1.0.0-rc.5.json`](release/1.0.0-rc.5.json), and the portable profile
remains a complete, self-contained, weaker contract.

The hardened contract is all-or-nothing. Under a qualified platform, the kernel
or hypervisor denies every network operation, every mutation of the frozen
snapshot and `GOROOT`, every write outside the operation-private build root,
and every execution outside the exact fingerprinted allowlist, and it enforces
aggregate process, memory, disk, time, descriptor, and output bounds over the
whole build domain.

The profile states two boundaries and does not conflate them. Every capability,
qualification, and trusted-computing-base rejection happens **before the domain
is entered**, so an unsupported host never creates a build domain. The in-domain
guarantee self-test necessarily runs after entry, performed by the first process
inside the domain, and completes **before any package byte is read by any
process in the domain**. Both are strictly before `go list`, before `go build`,
and before any compiler. There is no partial hardened mode, no best-effort
guarantee, and no silent fallback to the portable profile.

The process graph gains one uncontained node between the manager parent and the
worker, the hardened supervisor, because creating a containment object and
applying its controls requires operations the contained domain must not be able
to perform. Verifying containment is the mirror case and belongs to the
contained worker, which is why the in-domain self-test follows domain entry
rather than preceding it. The trusted computing base grows to include the
supervisor
bytes, the enforcement-backend adapters, the capability probes, the domain
session channel, and the operating-system kernel primitives those
adapters use. An implementation MUST name all of them cryptographically in the
closed `hardened-tcb-v1` record: the manager parent, supervisor, and worker
digests, the observed operating-system kernel identity, its bounded release, and
its required `curator-hardened-host-build-v1` kernel build identity, the
enforcement backend with its observed version and the configuration
settings the qualification depends on, the fingerprinted toolchain, and every
additional mutable component — an interpreter, an installed package tree, a
script, a shared library, a policy file — as a closed
`{kind, name, algorithm, content_sha256}` record. There is no field that accepts
a trusted component as free text, no field that accepts the observed kernel as
free text, and reporting a record narrower than the base
actually relied on is `hardened_tcb_identity_invalid`. The point is
mechanical: two materially different trusted bases must not be able to produce
one digest, because that digest is what binds cache reuse, receipts, markers,
and claims. That is why the kernel build identity is a digest over the
platform's declared build-identity sources rather than an optional descriptive
string: a rebuilt or vendor-patched kernel reporting the same platform, release,
and build identifier is a different trusted base and gets a different record.
The same completeness rule governs the end of the operation: the domain is
destroyed and joined first, and the manager then recomputes the whole record and
requires byte identity before anything is published.

The profile does not claim that adversarial Go source is safe, that the Go
toolchain is free of defects, that the compiled artifact is authenticated, that
a same-principal or administrator adversary is excluded, or that side channels
are closed. The compile-only rejection policy above remains mandatory and is not
relaxed by containment.

**No platform is qualified in this revision.** Linux, macOS, and Windows each
carry a declaration naming an enforcement backend and its candidate public
primitives, and each is `unqualified` pending native adversarial evidence. The
macOS declaration records unescapable domain membership, atomic domain
termination, and aggregate resource bounds as blocking; the Windows declaration
records per-path execution allowlisting and the aggregate write-byte bound as
blocking. Every one of those restates the `no-private-aggregate-domain` finding
that produced the portable profile. Consequently every host rejects the hardened
profile with `hardened_profile_unsupported`, and no hardened conformance claim
can be emitted.

The hashed build input carries three identities: the execution policy
`hardened-worker-v1`, the profile identity `hardened-profile-v1`, and the
domain-separated `curator-hardened-tcb-v1` digest of the closed
`hardened-tcb-v1` record. Hardened and portable cache entries, receipts,
markers, and claims therefore cannot alias, and cache reuse cannot cross a
profile revision or a trusted computing base: an artifact carries exactly the
guarantees of the trusted code that produced it, and a supervisor or worker
update forces a rebuild rather than a silent inheritance. Hardened builds write
receipt schema 3 or 4, marker schema 4, and claim schema 4, each carrying all
three identities; the portable schemas keep their exact bytes and are not
widened.

The one thing deliberately left out of every reusable output is the
per-operation `hardened-capability-evidence-v1` record. It reports what a single
operation observed, so hashing it would invite reading a cache key as capability
proof.
...
`skill-build.json` is untrusted compiler-selection data. It MAY select only a

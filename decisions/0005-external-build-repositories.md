...
`curator-build.json` schema 1, and the closed driver identifier
`go-repository-v1`. They are a new rc.5 protocol surface.

Schemas 1 through 6, `go-v1`, receipt schema 1, marker schemas 1 and 2,
conformance claim schemas 1 and 2, and protocol `1.0.0-rc.4` are frozen. A
reader or writer MUST NOT reinterpret, widen, relabel, or infer any rc.5
meaning from an rc.4 object. Schemas 1 through 6 MUST reject every schema-7
field and `go-repository-v1`.
...
one logical target from that repository's root `curator-build.json`.
...
source state. Marker schema 3 MUST represent both versions explicitly and MUST
NOT infer receipt meaning from a driver name. Marker v3 top-level

...
(STORY-260921-3z0fgr): the Pi dangling-link operator evidence
(Context, choice 3) and the absent-`cli_auth_credentials_store`-key
resolves to effective `file` rule (choice 4); normative text amended
...
Operator evidence 2026-09-21 (correction revision): on the operator
Mac the managed Pi home links to `~/.pi/auth.json`, which does not
exist — the real credential is `~/.pi/agent/auth.json` — so the link
is dangling, and `env status` does not report it. A dangling or
mis-targeted credential link is evidence, not a hypothesis, and must
be reported instead of silence (choice 3). On the operator Mac, ~/.curator/environments/default/pi/auth.json points to ~/.pi/auth.json; that target does not exist, the real credential is ~/.pi/agent/auth.json, and env status does not show the dangling passthrough as detached. env status and resolve report dangling or mis-targeted credential links as detached, with environment_credential_conflict-class wording instead of silence.
